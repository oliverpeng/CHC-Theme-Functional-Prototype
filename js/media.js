    ...
    ajaxFetchPhotos({
        ...
        callback: function(json) {
            $.each(json.photos.photo, function(i, photo) {
                var thumbUrl = buildFlickrPhotoURL({farmId: photo.farm, serverId: photo.server, id: photo.id, secret: photo.secret, size: 's' });
                var $image = $( document.createElement('img') ).attr('src', thumbUrl);
                $( document.createElement('li') ).append( $image ).appendTo($ul);
            });
        }
    });    
